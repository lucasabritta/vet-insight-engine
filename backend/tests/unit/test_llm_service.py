...
from app.services.llm_service import (
    ...
    extract_structured_record_async,
)
...
@pytest.mark.asyncio
async def test_call_openai_with_retry_success(sample_extracted_data):
    ...
    with patch("openai.AsyncOpenAI") as mock_openai:
        ...
        result = await call_openai_with_retry("Test prompt")
        assert result == response_text


@pytest.mark.asyncio
async def test_call_openai_with_retry_timeout_recovery(sample_extracted_data):
    """Test OpenAI API call recovers after timeout."""
    response_text = json.dumps(sample_extracted_data)
    mock_response = MagicMock()
    mock_response.choices[0].message.content = response_text
    ...
    with patch("openai.AsyncOpenAI") as mock_openai:
        ...
        result = await call_openai_with_retry(
            "Test prompt", retry_config=RetryConfig(max_retries=2, backoff_factor=0.01)
        )
        assert result == response_text


@pytest.mark.asyncio
async def test_call_openai_with_retry_max_retries_exceeded():
    """Test OpenAI API raises error after max retries."""
...
@pytest.mark.asyncio
async def test_extract_structured_record_async_success(sample_extracted_data):
    """Test async extraction of structured record."""
    response_text = json.dumps(sample_extracted_data)
    ...
    with patch("openai.AsyncOpenAI") as mock_openai:
        ...
        record = await extract_structured_record_async("Sample raw text")
        assert isinstance(record, VeterinaryRecordSchema)
        assert record.pet.name == "Buddy"
